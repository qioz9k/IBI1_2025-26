...
working_directory = "D:/IBI/IBI1_2025-26-master/Practical10"
...
# In the first 10 years of Afghanistan data, the highest DALYs value occurs in year_max_afg.
print(f"\nYear with highest DALYs in first 10 years: {year_max_afg}") #Year with highest DALYs in first 10 years: 1998
# Import required libraries
import os
import pandas as pd
import matplotlib.pyplot as plt
...
# Zimbabwe DALYs records span from first_zim (first year) to last_zim (last year).
print(f"\nZimbabwe data ranges from {first_zim} to {last_zim}")#Zimbabwe data ranges from 1990 to 2019
...
# In 2019, the highest DALYs country is directly stored in country_max(Lesotho 90771.64),
# and the lowest DALYs country is stored in country_min(Singapore 15045.11).
print(f"\nIn 2019:")
...
# The following plot shows DALYs trend over time for the country with the highest DALYs in 2019.
...
plt.show()
# The script successfully reads and processes the questions from question.txt and prepares them for analysis.

#solution of my question
plt.figure(figsize=(10, 6))
plt.boxplot(data_2019['DALYs'], patch_artist=True, boxprops=dict(facecolor="lightblue"))
plt.title('Distribution of DALYs Across All Countries in 2019', fontsize=14)
plt.ylabel('DALYs Rate', fontsize=12)
plt.grid(axis='y', alpha=0.3)
plt.tight_layout()
plt.savefig('2019_DALYs_boxplot.png', dpi=300)
plt.show()